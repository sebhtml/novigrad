...
fn print_results<T>(
    ...
    let last_row = outputs[0].tensor().read().unwrap().rows() - 1;
    ...
    for i in 0..inputs.len() {
        ...
        let actual_output: &Tensor = &actual_output.tensor().read().unwrap();
        let perplexity = get_perplexity(actual_output, actual_output.rows() - 1)?;

        let expected_output: &Tensor = &outputs[i].tensor().read().unwrap();
...
pub fn total_metrics<T>(
    ...
    for i in 0..inputs.len() {
        ...
        let example_loss: &Tensor = &example_loss.tensor().read().unwrap();
