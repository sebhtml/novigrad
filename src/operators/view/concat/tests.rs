...
fn concat() {
    ...
    let output = device.tensor(2, 9, vec![0.0; 2 * 9]).unwrap();
...
fn unconcat() {
    ...
    let output_1 = device.tensor(2, 3, vec![0.0; 2 * 3]).unwrap();
    let output_2 = device.tensor(2, 3, vec![0.0; 2 * 3]).unwrap();
    let output_3 = device.tensor(2, 3, vec![0.0; 2 * 3]).unwrap();

    let inputs = &[&input];
    let outputs = &[&output_1, &output_2, &output_3];

    let expected_output_1 = device
        .tensor(
            2,
            3,
            vec![
